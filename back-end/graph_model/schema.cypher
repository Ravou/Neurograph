...
//      created_at, priority, source, ...})
// (CloudResource:CloudResource {id, type, provider, name, ...})
// (BusinessService:BusinessService {id, name, description, ...})
// (Category:Category {id, name, description})
// (SubCategory:SubCategory {id, name})
// (Urgency:Urgency {level, name, description})
// (Impact:Impact {level, name, description})
//
// Relations principales :
// (Incident)-[:RELATES_TO_SERVICE]->(BusinessService)
// (Incident)-[:AFFECTS]->(CloudResource)
// (Incident)-[:HAS_CATEGORY]->(Category)
// (Incident)-[:HAS_SUBCATEGORY1]->(SubCategory)
// (Incident)-[:HAS_URGENCY]->(Urgency)
// (Incident)-[:HAS_IMPACT]->(Impact)
// (User)-[:ASSIGNED_TO]->(Incident)
// (User)-[:REPORTED]->(Incident)
// (User)-[:MANAGES]->(Incident)
// (Incident)-[:BLOCKED_BY]->(Incident)
// (SubCategory)-[:BELONGS_TO_CATEGORY]->(Category)
// (CloudResource)-[:BELONGS_TO_SERVICE]->(BusinessService)
